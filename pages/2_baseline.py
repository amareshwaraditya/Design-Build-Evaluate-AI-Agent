...
from src.athena import ATHENA_NAME, COMPANY_NAME, BOUNDARY
...
st.title(f"{ATHENA_NAME} — Basic Customer Support")
st.subheader(f"Your digital support agent at {COMPANY_NAME}")
st.write(f"I am {ATHENA_NAME}, the first working version of the smart customer-service agent at {COMPANY_NAME}. I accept your message, classify it using simple rules, and return a helpful template response.")
st.caption(BOUNDARY)
...
st.subheader("Chat with Athena")
for turn in bot.turns:
    with st.chat_message("user"):
        st.write(turn.user)
    with st.chat_message("assistant", avatar="🤖"):
        st.write(turn.assistant)
        st.caption(f"Athena’s current basic intent: `{turn.intent}`")
...
with col1:
    if st.button("Start a new customer conversation"):
        st.session_state.phase2_bot = BaselineChatbot()
        st.rerun()
with col2:
    st.download_button("Download conversation summary", str(bot.summary()), file_name="athena_basic_support_summary.txt")

st.subheader("Athena’s current session")
st.json(bot.summary())
st.subheader("When Athena’s basic architecture needs improvement")
